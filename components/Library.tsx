...
export const Library: React.FC<LibraryProps> = ({ onOpenBook }) => {
  ...
  // We keep this to prevent default browser behavior for file drops,
  // but the visual overlay and specific drag-drop handling for upload is removed.
  useDragDropController({
    enableDragOverlay: false, // Already false, but explicitly keeping it
  ...
  // Filter books based on search query, file type, and tags
  useEffect(() => {
    let filtered = books;
  ...
  }, [books, searchQuery, selectedFileType, selectedTags]);
  ...
  // No longer adding a placeholder for drag and drop
  const getGridItems = () => {
            ...
            <div className="flex items-center gap-4 flex-wrap flex-1">
              ...
              <div className="relative min-w-[250px] flex-1">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-1.5 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300"
                  style={{
                    backgroundColor: interfaceStyles.backgroundColor,
                    borderColor: interfaceStyles.borderColor,
                    color: interfaceStyles.color
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium opacity-70" style={{ color: interfaceStyles.color }}>
                  {getTranslation('fileType')}:
                </span>
                <select
                  value={selectedFileType}
                  onChange={(e) => setSelectedFileType(e.target.value)}
                  className="px-3 py-1.5 rounded-lg border text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                  style={{
                    backgroundColor: interfaceStyles.backgroundColor,
                    borderColor: interfaceStyles.borderColor,
                    color: interfaceStyles.color
              ...
              {allTags.length > 0 && (
                <div className="flex items-center gap-2">
                  ...
                  <select
                    value={selectedTags.length === 1 ? selectedTags[0] : (selectedTags.length === 0 ? 'all' : 'multiple')}
                    onChange={(e) => {
                  ...
                  >
                    <option value="all">{getTranslation('allTags')}</option>
                    {allTags.map(tag => (
                      <option key={tag} value={tag}>
                        {getTranslation(tag.toLowerCase()) || tag}
                      </option>
                    ))}
                    {selectedTags.length > 1 && <option value="multiple">{getTranslation('multipleSelected')}</option>}
                  </select>
                ...
                >
                  ...
                  >
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setExpandedMetadata(expandedMetadata === book.id ? null : book.id);
                      }}
                      className="absolute bottom-2 right-2 bg-black bg-opacity-50 text-white p-1.5 rounded-lg hover:bg-opacity-70 transition-all duration-300 opacity-0 group-hover:opacity-100"
                      title={getTranslation('showMetadata')}
                    >
                  ...
                  {expandedMetadata === book.id && (
                    <div 
                      className="px-4 py-3 border-t text-xs space-y-2 animate-fade-in"
                      style={{ backgroundColor: preferences?.interfaceColors?.panelSecondaryBackground || '#f9fafb' }}
                    >
                      <h4 className="font-bold uppercase tracking-wider opacity-50">{getTranslation('metadata')}</h4>
                      {book.metadata?.publisher && (
                        <div><span className="font-semibold">{getTranslation('publisher')}:</span> {book.metadata.publisher}</div>
                      )}
                      {book.metadata?.pubdate && (
                        <div><span className="font-semibold">{getTranslation('pubdate')}:</span> {book.metadata.pubdate}</div>
                      )}
                      {book.metadata?.language && (
                        <div><span className="font-semibold">{getTranslation('language')}:</span> {book.metadata.language}</div>
                      )}
                      {book.metadata?.tags && book.metadata.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1">
                          {book.metadata.tags.map((tag: string) => (
                            <span key={tag} className="px-1.5 py-0.5 bg-blue-100 text-blue-700 rounded text-[10px]">
                              {getTranslation(tag.toLowerCase()) || tag}
                            </span>
                          ))}
                        </div>
                      ...
                      {book.metadata?.description && (
                        <div className="mt-2 text-gray-600 italic line-clamp-3">
                          {book.metadata.description}
                        </div>
                      )}
                    </div>
